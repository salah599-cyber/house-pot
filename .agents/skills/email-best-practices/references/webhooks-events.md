...
- **Verify webhook signatures before any side effects**
- Handle duplicate events (idempotent processing)

Use the **raw request body** for verification. If your framework parses JSON first, signature checks will fail or be bypassed.
...
// Preserve raw bytes — do not use express.json() on this route
app.post(
  '/webhooks/resend',
  ...
  async (req, res) => {
    ...
    // Acknowledge only after verification
    res.status(200).send('OK');

    // Process verified payload asynchronously
    processWebhookAsync(event).catch(console.error);
  }
);
```

Never process `req.body` before verification — unverified webhooks can spoof bounces and complaints, causing erroneous list suppression.
