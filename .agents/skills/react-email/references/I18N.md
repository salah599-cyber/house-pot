...
**Security:** Never pass user-controlled `locale` values directly into dynamic import paths. Allowlist supported locales and load messages via static import maps (`locales.ts`):
...
export async function loadMessages(locale: string) {
  ...
  return (await messageLoaders[safeLocale]()).default;
}
```
...
import { loadMessages, resolveLocale } from '../locales';
import {
  Html,
  Head,
  Preview,
...
export default async function WelcomeEmail({
  ...
  const safeLocale = resolveLocale(locale);
  const welcomeEmailLoaders = {
    en: () => import('../messages/en/welcome-email.json'),
    es: () => import('../messages/es/welcome-email.json'),
    fr: () => import('../messages/fr/welcome-email.json'),
  } satisfies Record<Locale, () => Promise<{ default: unknown }>>;

  const { formatMessage } = createIntl({
    locale: safeLocale,
    messages: (await welcomeEmailLoaders[safeLocale]()).default
  });
...
i18next
  ...
  .use(resourcesToBackend((language, namespace) => {
    if (!i18next.options.supportedLngs?.includes(language)) {
      return Promise.reject(new Error(`Unsupported language: ${language}`));
    }
    return import(`./messages/${language}/${namespace}.json`);
  }))
  .init({
...
export async function getT(namespace, locale) {
  const safeLocale = resolveLocale(locale ?? 'en');
  if (i18next.resolvedLanguage !== safeLocale) {
...
export default async function WelcomeEmail({ name, locale }: WelcomeEmailProps) {
  const safeLocale = resolveLocale(locale);
  const { t } = await getT('welcome-email', safeLocale);
...
### 1. Always Pass and Validate Locale

Make locale a required prop and validate it against an allowlist before loading message files:

```tsx
interface EmailProps {
  locale: string; // Must be one of SUPPORTED_LOCALES
  // other props...
}

const safeLocale = resolveLocale(props.locale);
```
...
const safeLocale = resolveLocale(locale);
const t = createTranslator({
  messages: await loadMessages(safeLocale).catch(async () =>
    (await messageLoaders.en()).default
  ),
  locale: safeLocale,
  namespace: 'welcome-email'
});
...
export default async function OrderConfirmation({
  ...
  const isRTL = ['ar', 'he'].includes(safeLocale);
